"""
Planning & Synthesis Agent

This agent is responsible for:
- Generating high-quality hypotheticals to drive retrieval
- Building tightly-scoped prompts
- Calling an LLM (Gemini or OpenAI) to synthesize structured answers
"""
...
class PlanningSynthesisAgent:
    def __init__(self, retrieval_service=None, model_name: str = "gemini-2.5-flash") -> None:
        ...
        if self.provider == "gemini":
            ...
            # Default to a common lightweight model if caller keeps the Gemini-style default
            if self.model_name == "gemini-2.5-flash":
                self.model_name = "gpt-4.1-mini"
            print(f"OpenAI model '{self.model_name}' initialized successfully.")
        else:
            raise ValueError(f"Unsupported LLM_PROVIDER: {self.provider}")

    async def _generate_text(self, prompt: str) -> str:
        """Unified LLM caller that supports Gemini (streaming) and OpenAI chat completions.""" 
        if self.provider == "gemini":
            response = await self._gemini_model.generate_content_async(prompt)
            return response.text
        elif self.provider == "openai":
            # Minimal async OpenAI client usage
    ...
    async def synthesize_batch_answers(self, questions: List[str], contexts_per_question: List[List[str]]) -> List[str]:
        """
        Builds a batched prompt, sends it to the LLM API, and parses the response.
        ...
        print(f"--- Calling {self.provider.upper()} API (batched) ---")
        try:
            raw = await self._generate_text(batched_prompt)
...
async def synthesize_final_answer(self, question: str, context_chunks: List[str]) -> str:
        """
        Builds a single prompt, sends it to the LLM API, and returns the 'answer' string.
        """
        prompt = self.build_gemini_prompt(question, context_chunks)

        print(f"--- Calling {self.provider.upper()} API (single) ---")
        try:
        ...
        try:
            ...
            if isinstance(parsed, dict) and "answer" in parsed:
                return parsed.get("answer")  # type: ignore[arg-type]
